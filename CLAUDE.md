...
ClaudeMonitor は Claude AI と OpenAI Codex の使用量を1アプリでリアルタイム監視するWindows専用デスクトップウィジェット。Go + WebView2 で実装され、プロバイダーごとの短期・長期ウィンドウ、次回リセット時刻、追加利用枠、サービス障害を表示する。
...
ウィンドウはタブ表示が **230 × 320px**、1画面表示が **460 × 320px**（`main.go` のレイアウト別定数）。バナーやタブが増えると下部が `overflow: hidden` で切れる。
...
|---------|:-----------:|:-------------:|:--------:|---------|
| 通常（ログイン済・障害なし） | — | — | — | provider-tabs + row-bar 2本、余裕あり |
| ログイン済・overage表示・障害あり | — | ✓ | ✓ | provider-tabs + row-bar 3本、各33px以上 |
| 未ログイン | ✓ | — | — | provider-tabs + auth-banner + row-bar 2本、各33px以上 |
...
新規バナー要素を追加するときは、既存バナーとの同時表示でも281px（コンテンツ使用可能高さ）に収まるかピクセル計算で確認すること。row-barの最小高さは約33px（padding6+bar12+gap2+テキスト9+border2）。
