...
# Windows リソースファイル再生成（アイコン変更後に必要）
...
ウィンドウは **230 × 260px 固定**。バナーが増えると下部が `overflow: hidden` で切れる。
UI変更・バナー追加時は必ず以下の組み合わせでレイアウトが収まるか確認すること。

| シナリオ | auth-banner | status-banner | 期待結果 |
|---------|:-----------:|:-------------:|---------|
| 通常（ログイン済・障害なし） | — | — | 全要素表示 |
| ログイン済・障害あり | — | ✓ | 全要素表示（row-bar が約53pxに縮小） |
| 未ログイン | ✓ | — | 全要素表示（row-bar が約42pxに縮小） |
| 未ログイン・障害あり | ✓ | **非表示** | auth優先でstatus-bannerを隠す（設計による制御） |

**設計ルール**: `authState !== 'ok'` のとき `status-banner` は非表示にする（`renderStatusBanner` 内で制御）。auth-banner と status-banner を同時表示すると約16pxはみ出してfooter/links-rowが切れるため。

新規バナー要素を追加するときは、既存バナーとの同時表示でも221px（コンテンツ使用可能高さ）に収まるかピクセル計算で確認すること。
